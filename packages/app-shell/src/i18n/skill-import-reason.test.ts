...
describe("skill import copy", () => {
  it("keeps every import status and reason key in both locales", () => {
    ...
    const zhKeys = Object.keys(translationResources["zh-CN"]).filter((key) => key.startsWith(prefix));
    const enKeys = Object.keys(translationResources["en-US"]).filter((key) => key.startsWith(prefix));
  ...
  it("localizes importing and failed states in the active language", async () => {
    ...
    expect(localizeSkillImportReason("skill_storage_error", t)).toBe("无法写入技能文件。");
    expect(localizeSkillImportReason("not_a_real_code", t)).toBe("导入失败。");
    expect(localizeSkillImportResultReason({ status: "failed", errorCode: null }, t)).toBe("导入失败。");
    ...
    expect(localizeSkillImportReason("skill_storage_error", t)).toBe("The skill files could not be written.");
    expect(localizeSkillImportReason("not_a_real_code", t)).toBe("Import failed.");
    expect(localizeSkillImportResultReason({ status: "failed", errorCode: null }, t)).toBe("Import failed.");
    expect(localizeSkillImportStatus("committing", t)).not.toBe("导入中");
    expect(localizeSkillImportReason("skill_storage_error", t)).not.toBe("无法写入技能文件。");
  });

  it("does not leak raw English status enums", async () => {
    await appI18n.changeLanguage("zh-CN");
    expect(localizeSkillImportStatus("committing", appI18n.t.bind(appI18n))).not.toBe("committing");
    expect(localizeSkillImportStatus("mystery", appI18n.t.bind(appI18n))).toBe("未知状态");
  });
});
