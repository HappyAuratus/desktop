...
- Direct requests fail when the stream closes before their oneshot completes. Abandoned session requests leave bounded correlation tombstones so their late responses can be discarded without masking genuinely unknown ids.
- Unmatched response ids, invalid response envelopes, and stdio loss remain fatal to the connection.
