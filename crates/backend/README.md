...
- `Backend::open` creates required directories, bootstraps and migrates SQLite, reconciles imported skill packages (catalog rows whose on-disk package is missing stay unavailable instead of refusing to start), constructs APIs, starts the [agent runtime](src/agent_runtime/README.md), and composes the workflow run engine.
