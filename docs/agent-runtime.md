...
ACP stdout is newline-delimited JSON-RPC with an 8 MiB frame limit. The connection reader uses an unbounded ordered event handoff to the always-running central router, while each registered Session owns one bounded 256-item FIFO for updates, permission requests, and terminating responses, plus an independent control queue for connection loss and overflow. This keeps connection-wide parsing from imposing one Session's backpressure on another and preserves wire order so a response cannot overtake its turn's tail updates. Active actors drain events already queued ahead of a terminal control, but check commands after bounded event batches so a live update stream cannot starve cancellation or stop. A per-Session overflow stops only the affected Session; no data is silently discarded.

Unknown agent-originated JSON-RPC requests receive a correlated `-32601` method-not-found response and do not terminate the connection. Malformed frames, genuinely unmatched response ids, oversized frames, and stdio loss are connection failures. Abandoned session request ids remain as bounded correlation tombstones so their late responses can be discarded; responses routed before abandonment completes are filtered by request identity in the actor. Routes are generation-bound, so updates from an old connection or an unloaded Session are treated as stale and discarded rather than taking down unrelated work.
